...
# ─── Detect previously-installed backend ────────────────────────────────────────
# The installed binary embeds the backend choice via compile-time defines.
# We detect it by checking if the binary was built with USE_YTCUIDL
# (strings output contains "ytcui-dl" in that case). Default to ytcuidl.
BACKEND="ytcuidl"
if strings "$INSTALL_PREFIX/bin/$BINARY_NAME" 2>/dev/null | grep -q "ytcui-dl"; then
    BACKEND="ytcuidl"
elif strings "$INSTALL_PREFIX/bin/$BINARY_NAME" 2>/dev/null | grep -q "yt-dlp"; then
    BACKEND="ytdlp"
fi
info "Rebuilding with backend=$BACKEND"

# ─── Build ──────────────────────────────────────────────────────────────────────
info "Building..."
$MAKE_CMD clean >/dev/null 2>&1 || true
$MAKE_CMD BACKEND="$BACKEND" || die "Build failed"
ok "Build complete"

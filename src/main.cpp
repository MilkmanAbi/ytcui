...
// ytcui-dl: RAII curl global init (must live for process lifetime)
#ifdef USE_YTCUIDL
#include "ytcui-dl/ytfast.h"
static ytfast::CurlGlobalInit g_curl_init;
#endif
#include <cstdio>
#include <cstring>
#include <csignal>
...
static void run_diag(const Ansi& C) {
    ...
    printf("  %sBackend:%s ytcui-dl (built-in, yt-dlp not required)\n\n",
           C.GREEN, C.RESET);
#else
    Dep deps[] = {
...
int main(int argc, char* argv[]) {
    ...
    // ── OIS lifecycle flags: route to the OIS hook installed alongside us ──
    // When ytcui was installed via OIS, an executable hook named ".ytcui-ois"
    // sits next to the binary and forwards to OIS (install/uninstall/update/...).
    // We exec it so `ytcui --update`, `--uninstall`, `--ois`, etc. just work.
    for (int i = 1; i < argc; i++) {
        const char* a = argv[i];
        if (strcmp(a, "--ois") == 0 || strcmp(a, "--update") == 0 ||
            ...
            std::string cands[3] = {
                dir.empty() ? std::string() : dir + "/.ytcui-ois",
                "/usr/local/bin/.ytcui-ois",
                std::string(getenv("HOME") ? getenv("HOME") : "") + "/.local/bin/.ytcui-ois"
            };
            for (const auto& hook : cands) {
                if (!hook.empty() && access(hook.c_str(), X_OK) == 0) {
                    // Forward the flag (and any trailing args) to OIS.
                    std::vector<char*> av;
                    av.push_back(const_cast<char*>("sh"));
                    av.push_back(const_cast<char*>(hook.c_str()));
                    for (int j = i; j < argc; j++) av.push_back(argv[j]);
                    av.push_back(nullptr);
                    execvp("sh", av.data());
                    perror("ytcui: failed to launch OIS");
                    return 1;
                }
            }
            fprintf(stderr,
                "ytcui: '%s' is managed by OIS, but no OIS install was found.\n"
    ...
    for (auto& a : args) {
        if (a == "--colors" || a == "--colour" || a == "--color-help") {
            printf("\n%s ytcui colour elements%s\n\n", C.BOLD, C.RESET);
            printf("  Each element can be set to a 256-colour index (0–255) or -1 for terminal default.\n");
            printf("  Custom colours are set in %s~/.config/ytcui/config.json%s under \"colors\": {}\n", C.CYAN, C.RESET);
...
#ifdef USE_YTCUIDL
        fprintf(stderr, "Error: ytcui-dl backend failed to initialize.\n");
#else
    ...
    // ── Orderly teardown ──────────────────────────────────────────────────
    // Stop the prefetch worker and join the update-check thread so no thread
    // is alive when curl_global_cleanup() (g_curl_init dtor) and the singleton
    // teardown run. This is the fix for the macOS "malloc: pointer being freed
    // was not allocated" abort that appeared after a search.
